...
export default function App() {
  ...
  useEffect(() => {
    const manifest = {
      ...
      start_url: ".",
      display: "standalone",
      orientation: "landscape", // 【核心优化】：PWA 安装后强制进入横屏模式
      theme_color: "#fafaf9",
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      } catch (e) { console.error("读取本地记忆失败", e); } finally { setIsLoaded(true); }
    };
    initData();
  }, []);
  ...
  const loadHistoryList = async () => {
    ...
    } catch (e) { console.error("加载记忆列表失败", e); }
  };
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(async () => {
      try {
        ...
        await dbHelper.put({ 
          id: activeRecordId, name: activeRecordId === 'draft' ? '当前草稿' : activeRecordName,
          pins, ranges, hoursCount, order: activeRecordId === 'draft' ? 0 : currentOrder
        });
  ...
  const handleDeleteHistory = async (id, e) => {
    ...
    try {
      ...
      if (activeRecordId === id) await handleBackToDraft(); else await loadHistoryList();
    } catch (err) { }
  };
  ...
  const handlePointerMove = (e) => {
    ...
    if (draggingLabel) {
      const dx = e.clientX - draggingLabel.startX; const dy = e.clientY - draggingLabel.startY;
  ...
  return (
    ...
    >
      ...
      {/* 【核心优化 2：画布容器居中靠下适应全屏】 */}
      <main className={`flex-1 relative w-full overflow-hidden flex flex-col justify-start pt-[5vh] ${showGallery ? 'invisible' : 'visible'}`}>
        <div className="w-full overflow-x-auto pb-10 pt-10 custom-scrollbar outline-none" ref={trackRef}>
          {/* 高度加大到 600px 给上下元素留空间 */}
          <div className="relative h-[600px] mx-12 min-w-[800px]" ref={containerRef} style={{ width: `${totalWidth + 120}px` }}>
            
            {/* 【核心优化 4：极限收窄画线判定区】高度只有 40px，必须精准点在时间线上 */}
            ...
            {trackDragState && (
              <div className="absolute z-50 pointer-events-none flex flex-col items-center transition-none" style={{ left: `${trackDragState.currentMins * PIXELS_PER_MINUTE}px`, top: '150px', bottom: '0', width: '2px' }}>
                <div className="w-[2px] h-full bg-red-400 opacity-60" />
                <div className="absolute top-[30px] bg-red-500 text-white text-[11px] px-2 py-0.5 rounded-full shadow-md font-mono whitespace-nowrap">
            ...
            <svg className="absolute inset-0 w-full h-full pointer-events-none z-50 overflow-visible">
              {connectingPin && (
                <line x1={connectingPin.time * PIXELS_PER_MINUTE} y1={BASE_Y - 16} x2={dragCurrentPos.x} y2={dragCurrentPos.y} stroke="#a8a29e" strokeWidth="2" strokeDasharray="6 4" strokeLinecap="round" />
              )}
            </svg>
            ...
            {leveledRanges.map(range => {
              ...
              const topPos = BASE_Y - 16 - range.level * 45; 
                ...
                <div key={range.id} onPointerDownCapture={() => setActiveZId(range.id)} className={`absolute -translate-y-1/2 group transition-all duration-300 interactive-element ${activeZId === range.id ? 'z-50' : 'z-30'}`} style={{ left: `${leftPos}px`, width: `${width}px`, top: `${topPos}px` }}>
                  ...
                  <div className="absolute left-1/2 top-0" style={{ width: 0, height: 0 }}>
                    ...
                    <div className="absolute flex flex-col items-center group/label z-10 touch-none" style={{ left: `${offset.x}px`, top: `${offset.y}px`, transform: 'translate(-50%, -50%)' }}>
                      {/* 【核心优化 3：超大文本拖拽区域】高度大幅增加，全宽，保证手指一触即发 */}
                      <div className="w-[120%] pt-4 pb-3 -mt-6 cursor-move flex justify-center items-center touch-none" onPointerDown={(e) => handleLabelPointerDown(range, 'range', e)}>
                        <div className="w-8 h-1.5 bg-stone-300/60 rounded-full group-hover/label:bg-stone-300 transition-colors" />
                      </div>
            ...
            {pins.map(pin => {
              ...
              return (
                <div key={pin.id} onPointerDownCapture={() => setActiveZId(pin.id)} className={`absolute interactive-element transition-all duration-300 ${activeZId === pin.id ? 'z-50' : 'z-40'}`} style={{ top: `${BASE_Y}px`, left: `${leftPos}px` }}>
                  ...
                  <div className="absolute flex flex-col items-center group/label z-10 touch-none" style={{ left: `${offset.x}px`, top: `${offset.y}px`, transform: 'translate(-50%, -50%)' }}>
                    {/* 【核心优化 3：超大文本拖拽区域】 */}
                    <div className="w-[120%] pt-4 pb-3 -mt-6 cursor-move flex justify-center items-center touch-none" onPointerDown={(e) => handleLabelPointerDown(pin, 'pin', e)}>
                      <div className="w-8 h-1.5 bg-stone-300/60 rounded-full group-hover/label:bg-stone-300 transition-colors" />
                    </div>
                    <div className="relative flex justify-center items-center">
      ...
      <footer className="p-8 pb-8 flex justify-center items-center pointer-events-none shrink-0 z-40">
        <p className="text-stone-400 italic text-sm transition-all duration-700 ease-in-out">{ambientMessage}</p>
      </footer>
